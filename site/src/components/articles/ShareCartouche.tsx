  ...
  return (
    <div className="share-block">
      ...
      <div className="share-row">
        ...
        <a className="share-icon" href={`mailto:?subject=${encodedTitle}&body=${encodedPlainMessage}`} aria-label="Email">
          <svg viewBox="0 0 24 24" aria-hidden="true">
            <rect x="2.5" y="4.5" width="19" height="15" rx="2" />
            <path d="m3.5 6 8.5 7 8.5-7" />
          </svg>
        </a>
        {/* SMS repris à sa vraie place (retour validation humaine, 13/09) —
            la maquette l'affiche comme un bouton normal parmi les 8, jamais
            en dernier ni masqué sur desktop (voir .v2-jc-page
            .share-icon-sms dans globals.css, qui annule ici seulement la
            règle @media(hover:hover) — décision produit distincte du 05/09,
            non remise en cause ailleurs — RM/Connaître Jésus/Livres restent
            inchangés ce tour-ci). */}
        <a className="share-icon share-icon-sms" href={`sms:&body=${encodedPlainMessage}`} aria-label="SMS">
          <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        {/* Icône "copier" (retour du 05/09, 4e passage) — glyphe conservé
            (déjà conforme au glyphe "copy" de la maquette), mais le style
            inline qui retirait le cadre/le fond était une régression
            propre à ce bouton, jamais présente dans la maquette (son
            bouton "copy" porte le même .share-icon encadré que les
            7 autres) — retiré. */}
        <button
          className="share-icon"
          onClick={() => {
