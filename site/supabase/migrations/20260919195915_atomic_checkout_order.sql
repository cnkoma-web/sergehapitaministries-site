-- Création atomique de la commande et de ses lignes. La fonction reste
-- SECURITY INVOKER et n'est appelable que par le service_role : elle n'ouvre
-- donc aucune écriture privilégiée au navigateur.
...
begin
  ...
  if exists (
    ...
    where item.title_snapshot is null
  ...
  insert into public.order_items (
    order_id, book_id, goodie_id, title_snapshot,
    unit_price_cents, quantity, variant_size, variant_color
  )
  select
    v_order_id, item.book_id, item.goodie_id, item.title_snapshot,
    item.unit_price_cents, item.quantity, item.variant_size, item.variant_color
  from jsonb_to_recordset(p_items) as item(
    book_id uuid,
    goodie_id uuid,
    title_snapshot text,
