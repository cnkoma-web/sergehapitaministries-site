...
- **Version 3** (celle-ci) — Vérification après application de la v2 : le hub Publications n'a toujours pas été mis en flux unifié (à refaire, pas un changement d'avis) ; nouvelle section de rappel Rosée Matinale sous le flux ; précision affinée sur la largeur de colonne (alignée sur le menu de navigation, ~720-740px, toutes pages sauf accueil) qui remplace la règle v2 ; correctifs du widget Rosée Matinale et du hero Publications ; effet de survol (rollover) toujours non constaté, à reconfirmer avec Claude Code ; 4 bugs de l'éditeur de texte riche dans l'admin (retours à la ligne, champ verset complémentaire, suppression de tag, ergonomie des versets complémentaires).

**À partir de cette version, chaque nouvelle vague de modifications significatives donne lieu à un nouveau numéro de version et un nouveau nom de fichier** (`cahier-des-charges-technique-v3.md`, etc.) — l'ancien fichier n'est jamais écrasé silencieusement, pour garder un historique clair de l'évolution du projet.
