...
**Version 4 — 7 septembre 2026**

**Historique des versions :**
- **Version 1** — Document initial remis à Claude Code : maquette statique complète (22 pages), fondations globales, inventaire des pages, règles métier de base.
- **Version 2** — Ajouts suite aux retours d'utilisation réelle du site déployé et de l'admin : phase d'ajustements de l'admin (modèles de liste/édition/tableau de bord), correction du hub Publications (flux unifié, date/heure, pagination), règle de largeur de colonne unique, visuels de partage social (Open Graph), correctifs du teaser Publications de l'accueil (respiration, lisibilité, images, nombre d'articles réglable), retrait de l'agenda fictif (décision en attente).
- **Version 3** — Vérification après application de la v2 : le hub Publications n'a toujours pas été mis en flux unifié (à refaire, pas un changement d'avis) ; nouvelle section de rappel Rosée Matinale sous le flux ; précision affinée sur la largeur de colonne (alignée sur le menu de navigation, ~695px mesuré précisément, toutes pages sauf accueil) qui remplace la règle v2 ; correctifs du widget Rosée Matinale et du hero Publications ; effet de survol (rollover) toujours non constaté à l'époque ; 4 bugs de l'éditeur de texte riche dans l'admin.
- **Version 4** (celle-ci) — Consolidation complète de la phase d'ajustements avancés qui a suivi la v3, couvrant : système de partage social multi-plateformes avec messages personnalisés par catégorie et par plateforme, images Open Graph dynamiques, effet de survol des couvertures (rollover), système "J'aime", nouvelle logique du compteur de vues, refonte de l'éditeur de texte riche (citation en bloc, citation mise en exergue, éditeurs unifiés), règles de mise en forme des métadonnées d'articles et des cartes d'aperçu, nouveau champ "Titre" pour Rosée Matinale, pages légales complétées, gabarit email de réinitialisation de mot de passe, corrections de bugs structurels. Voir Partie 7 à Partie 13, nouvelles dans cette version.

**À partir de la version 2, chaque nouvelle vague de modifications significatives donne lieu à un nouveau numéro de version et un nouveau nom de fichier** (`cahier-des-charges-technique-v4.md`, etc.) — l'ancien fichier n'est jamais écrasé silencieusement, pour garder un historique clair de l'évolution du projet.
...
- **Images insérées dans le corps d'un article** (celles que Serge ajoute lui-même dans le texte via l'éditeur riche — pas la couverture, pas les images de héros) : même principe qu'énoncé plus loin pour les couvertures (§6.4 — le système ajuste automatiquement, jamais à Serge de redimensionner lui-même). Taille cible : **72% de la largeur de la colonne de texte définie en §1.7**, centrée, appliquée automatiquement à l'insertion.
...
### 1.7 Largeur de colonne de contenu — précision v3 (remplace la règle v2)

**Cette règle remplace celle de la v2** — la demande a été affinée après un nouveau test en production, ce n'est pas une contradiction mais une précision.

**Nouvelle règle** : la colonne de contenu (celle qui contient le texte, les images, les informations) doit être **alignée sur la largeur visuelle du menu de navigation du header** — c'est-à-dire commencer là où débute le bouton "Accueil" à gauche, et se terminer là où finit le bouton "Contact" à droite. Serge estime cette largeur autour de **720-740px**, à ajuster précisément par Claude Code selon la largeur réelle du menu de navigation tel qu'il est actuellement rendu.

**Cette règle s'applique à toutes les pages sauf l'accueil**, qui garde sa mise en page actuelle (hero pleine largeur, sections successives).

**Sur la fiche livre spécifiquement** : en plus de l'alignement général, l'écart horizontal entre l'image de couverture et la colonne d'informations (prix, caractéristiques) à droite est actuellement trop large et doit être resserré.
...
**6.9 Teaser Publications de l'accueil — 4 correctifs constatés en production**
Observations de Serge sur le site déployé, vérifiées une par une dans les fichiers sources :
1. **Section "Soutenir" sans respiration** — bug confirmé dans la maquette source : `.give` n'avait aucun padding vertical défini, le texte touchait les bords haut et bas de la zone colorée. Corrigé dans la maquette (padding haut/bas ajouté) ; à reporter en production.
2. **Titres de publications illisibles sur fond sombre** — vérifié : la maquette source définit bien `color:#fff` (blanc) sur ces titres. Si la production affiche un texte sombre peu lisible, c'est un écart d'implémentation à corriger côté code, pas un défaut de la maquette.
3. **Aucune image d'aperçu sur le teaser Publications de l'accueil** — confirmé, absente de la maquette source (elle n'existait que sur le hub `publications.html`, pas ici). Une vignette a été ajoutée à chaque carte du teaser.
4. **Nombre d'articles affichés en aperçu par catégorie, actuellement figé en dur (3)** — doit devenir un réglage géré par Serge depuis l'admin (ex. dans les paramètres de la section "Textes" ou un nouvel écran dédié), pas une valeur codée en dur dans le composant. Serge doit pouvoir choisir combien d'articles afficher en aperçu, par catégorie, sur la page d'accueil.

**6.10 Vérification après application de la v2 — plusieurs points non résolus ou nouveaux**

1. **Hub Publications — le flux unifié n'a toujours pas été appliqué.** Vérifié sur le site déployé (capture d'écran) : la page affiche toujours l'ancienne structure en blocs séparés par catégorie ("Que Dit la Bible ?" avec ses cartes, présumément "La Vie Supérieure" en dessous). Ceci n'est **pas un changement d'avis** — c'est une instruction déjà donnée (§6.5) qui n'a pas été mise en œuvre. À refaire : un seul flux, toutes catégories mélangées, triées par ordre d'arrivée, chaque publication portant uniquement sa pastille de catégorie (QB/VS/RM), sans gros titre de section par catégorie, avec pagination.

2. **Nouveau, en plus du flux unifié** : sous ce flux, ajouter une section distincte (fond de couleur différente) qui reprend, sous forme de résumés (même format de carte que le flux principal), les dernières entrées de Rosée Matinale. Rosée Matinale continue d'apparaître aussi dans le flux mélangé du dessus — cette section est un rappel supplémentaire, pas un remplacement.

3. **Widget "Rosée Matinale" de l'accueil** : le texte colle au badge et au lien "Lire la pensée du jour" — pas assez d'espacement interne. La zone doit aussi être proportionnée pour paraître carrée plutôt que rectangulaire.

4. **Titre du hero sur `publications.html`** : retour à la ligne anormal du texte — la cellule de texte du hero doit être élargie pour correspondre à la largeur de la colonne de contenu définie en §1.7 (alignée sur le menu de navigation), pas une largeur plus étroite qui casse la mise en page du titre.

5. **Page Rosée Matinale (`rosee-matinale.html`)** : aucun changement structurel — navigation jour précédent/suivant et archive restent telles quelles. Seule la largeur de colonne de texte (§1.7) s'applique ici comme partout ailleurs.

6. **Effet de survol (rollover) sur les couvertures — toujours absent.** Déjà demandé précédemment (§6.4, avec référence à rochedy.com) mais non constaté en production sur aucune des zones concernées : catalogue Livres, fiche livre, Boutique, vignettes de l'accueil. Il faut confirmer explicitement avec Claude Code si l'instruction n'a pas été comprise techniquement ou simplement pas exécutée, et vérifier que l'effet est bien implémenté partout où une couverture/visuel produit est affiché — pas seulement sur la fiche produit individuelle.

7. **Bugs de l'éditeur de texte riche dans l'admin** :
   - Les retours à la ligne / paragraphes saisis disparaissent après un enregistrement, puis réapparaissent — comportement instable à corriger.
   - Le champ "verset complémentaire" ne s'affiche pas côté public pour les articles créés après le tout premier test (qui fonctionnait) — bug d'affichage incohérent entre articles.
   - Impossible de supprimer un thème/tag une fois ajouté — chaque tag doit avoir une croix cliquable pour le retirer (voir §6.3, gestion de liste par Serge).
   - Le champ "versets complémentaires" avec la convention "un par ligne, format Référence | Texte" est peu ergonomique — remplacer par des champs séparés et répétables (un champ Référence + un champ Texte, avec un bouton pour en ajouter d'autres), plutôt qu'une syntaxe à mémoriser dans une zone de texte unique.

---

## PARTIE 6 — État des lieux des étapes techniques listées en v1 (mise à jour v4)

*Toutes les étapes ci-dessous, listées comme "prochaines étapes" dans la v1, sont maintenant réalisées et en production, sauf mention contraire :*

1. ✅ **Hébergement + nom de domaine** — sergehapitaministries.org, hébergé sur Vercel Pro, dépôt GitHub dédié.
2. ✅ **Architecture CMS** — panneau d'administration sur-mesure (`/admin`), couvrant Livres, Boutique, Publications, Rosée Matinale, Vidéos, Avis, Ticker, Statistiques, Textes — modèle liste compacte + édition détaillée, voir Partie 5.
3. ✅ **CMS Rosée Matinale** — publication quotidienne, archivage automatique, pagination de l'archive (3 par page), nouveau champ "Titre" (voir Partie 12).
4. ✅ **CMS Publications/Livres/Boutique/navigation/textes** — gestion sans toucher au code.
5. ✅ **Authentification réelle** — Supabase Auth, création de compte, session, mot de passe oublié (voir Partie 11 pour le gabarit email), déverrouillage de La Vie Supérieure par compte.
6. ✅ **Panier + Stripe** — mode réel activé (bascule test → live effectuée), webhooks configurés, frais de port calculés par palier de poids (voir Partie 3 v1).
7. ✅ **Modération des avis** — file d'attente, validation, publication.
8. ✅ **Formulaires connectés** — contact, invitation, don, newsletter (MailerLite), avis.
9. ⚠️ **Boutique et Vidéos** — pages non terminées, affichent un message "Page en cours de réalisation" en attendant (décision explicite, pas un oubli).
10. ✅ **Sitemap.xml et robots.txt** — corrigé au passage de Rosée Matinale en URL par jour (voir Partie 12).
11. ✅ **Visuels Open Graph** — génération dynamique par page/article, voir Partie 7.

---

## PARTIE 7 — Système de partage social

### 7.1 Images de partage (Open Graph) — génération dynamique

Chaque publication, livre et page statique génère automatiquement sa propre image de partage (via le système `next/og` / `ImageResponse`), sans intervention manuelle de Serge. Caractéristiques :
- **Couleur de fond selon la catégorie** : violet pour Que Dit la Bible, bleu pour La Vie Supérieure, sarcelle pour Rosée Matinale.
- **Logo et nom de rubrique centrés ensemble** (pas seulement le logo), formant un seul bloc visuel, avec un trait fin de séparation juste après cet ensemble.
- **Logo utilisé** : version blanche dédiée (`logo-white.png`, hébergé à la racine du site), pas le logo violet standard — contraste nécessaire sur les fonds colorés.
- **Polices de marque intégrées** : titre en Fraunces (police d'affichage du site), reste en Manrope — jamais de police système générique.
- **Photo de couverture originale jamais utilisée à la place de l'image générée** — bug corrigé (une photo lourde, non optimisée, empêchait l'aperçu de s'afficher correctement sur certaines plateformes).
- **Limite connue et acceptée** : l'image de Rosée Matinale reflète l'entrée du jour courant, pas un jour précis choisi rétroactivement pour un ancien lien déjà partagé (limite de mise en cache des plateformes, non corrigible après coup).
- Ratio conforme aux standards (1200×630px) pour un rendu correct sur Facebook/LinkedIn sans recadrage.

### 7.2 Message de partage personnalisé — par catégorie et par plateforme

**Principe général** : tutoiement, prénom "Serge" (pas "Serge Hapita"), ton personnel plutôt qu'une fiche automatique de site.

**Modèle général (version WhatsApp/Telegram — supporte la mise en forme `*gras*`/`_italique_`) :**

```
Bonjour,

Serge partage avec toi [formule + date] : *« [Titre] »*

_[Extrait du chapeau, 2 lignes max, tronqué proprement]…_

👉 [Invitation selon catégorie] :
[lien]

Bonne lecture et _*demeure abondamment béni.*_
```

**Version SMS/X — même contenu, sans aucun symbole de mise en forme** (ces plateformes n'interprètent pas `*`/`_`).

**Formulations exactes par catégorie :**

| Catégorie | Formule + date | Invitation |
|---|---|---|
| Rosée Matinale | "la Rosée matinale du [jour] [mois]" | "Retrouve la pensée complète ici" |
| Que Dit la Bible | "la réflexion biblique du [jour] [mois]" | "Retrouve l'intégralité de la réflexion ici" |
| La Vie Supérieure | "l'enseignement du [jour] [mois]" | "Retrouve l'enseignement complet ici" |

*Date : jour + mois en toutes lettres, sans l'année.*

**Règles de troncature de l'extrait** : dernier mot complet avant ~120 caractères (ou 2 lignes), jamais coupé en plein mot, ajout de "…" uniquement si réellement tronqué. Généré automatiquement depuis le chapeau existant — aucun nouveau champ admin créé pour ça.

**Pour les livres** — modèle distinct, l'accroche (extrait de la description du livre) est volontairement conservée :

```
Bonjour,

Serge t'invite à découvrir son livre : *« [Titre] »*
...
### 7.3 Boutons de partage — plateformes et positionnement

**Avec message personnalisé (7.2)** : WhatsApp, Telegram, X (Twitter), SMS.
**Lien seul, sans message personnalisé** (limitation propre à la plateforme, Facebook/LinkedIn refusent tout texte pré-rempli accompagnant un lien) : Facebook, LinkedIn.
**Sans mise en forme, lien seul également** : Email, "Copier le lien" (icône Copy explicite, tooltip "Copier le lien", retour visuel temporaire "Lien copié !" après clic).

**Style visuel** : icônes SVG nues (pas d'emoji, pas de cercle/fond gris encapsulant), positionnées entre deux traits horizontaux fins. Taille tactile minimale de 44×44px sur mobile (norme d'accessibilité).

**Zone de fond dédiée** : la zone contenant la phrase d'invitation + les icônes de partage a un fond `#7B3FE4` (violet principal de la marque, même valeur que le badge de catégorie/survol de lien/icône vues) — les icônes de réseaux sociaux passent en **blanc** sur ce fond. Cette bande est distincte du fond blanc du texte et du fond lavande pâle de "Autres articles similaires".

**Phrase d'invitation au-dessus des icônes** : *"Bénis quelqu'un en partageant ce message."* — en gras **et** italique. Absente sur les fiches livres (icônes seules, sans phrase).

**Positionnement du bloc** : après le corps de l'article, avant les sections annexes (Prière/Aller plus loin/thématiques/Autres articles similaires) — voir Partie 10 pour l'ordre exact.

---

## PARTIE 8 — Effet de survol des couvertures (rollover) et liens sur les vignettes livres

### 8.1 Effet de survol — technique du fondu enchaîné

Sur toute vignette de livre (catalogue, fiche produit, accueil), la couverture avant et la quatrième de couverture sont superposées au même endroit (deux images empilées) : celle du dessus visible par défaut (opacité 1), celle du dessous invisible (opacité 0), avec une transition CSS douce sur l'opacité (~0,3-0,4s). Au survol, un fondu enchaîné révèle la deuxième image — jamais un remplacement brutal, un glissement ou une bascule.

**Boutique** : n'a pas cet effet (chaque produit n'a qu'une seule image en base actuellement) — nécessiterait l'ajout d'une deuxième image par produit pour l'activer.

### 8.2 Trois liens distincts sur chaque vignette de livre

1. **Image de couverture** → mène vers la fiche du livre (coexiste avec l'effet de survol).
2. **Titre du livre** → mène vers la fiche du livre.
3. **Nom de l'auteur ("Serge Hapita")** → mène vers la page **De Serge**, pas vers la fiche du livre.
4. **Nom de l'éditeur ("amDG")** → lien externe vers `https://www.amdgeditions.fr`, ouvert dans un nouvel onglet.

*Écriture correcte de la marque éditeur, toujours : **"amDG Éditions"** (a minuscule, m minuscule, DG majuscules, É majuscule accentué, ditions minuscule) — jamais tout en majuscules.*

---

## PARTIE 9 — Système "J'aime"

**Style retenu** : phrase d'invitation + bouton (icône cœur SVG + "J'aime (n)"), sans contour ni fond — dans le prolongement du style épuré des icônes de partage. *Décision ultérieure : la phrase d'invitation a finalement été retirée, ne garder que le bouton seul (icône cœur + "J'aime (n)").*

**Icône** : cœur (vide par défaut, se remplit de violet au clic) — choix retenu plutôt que "mains levées".

**Règle d'accès, calquée sur l'accès à la lecture déjà en place :**
- **Que Dit la Bible** et **Rosée Matinale** (articles publics) → accessible à tout le monde, sans compte, simple compteur qui s'incrémente.
- **La Vie Supérieure** (déjà réservé aux comptes connectés) → bouton réservé aux utilisateurs connectés.

**Comportement à sens unique** : un like par visiteur/compte, pas de "désaimer".

**Positionnement** : juste après le corps du texte de l'article, avant Prière/Aller plus loin/thématiques.

---

## PARTIE 10 — Métadonnées d'articles et cartes d'aperçu — règles de mise en forme

### 10.1 Sur la page article elle-même (Que Dit la Bible / La Vie Supérieure)

Deux lignes, en italique, même taille de police, espacement large entre éléments (~24-28px, pas de séparateur "·" ni "|") :
- **Ligne 1** : nom de l'auteur, temps de lecture, nombre de vues.
- **Ligne 2** : date seule, format complet en toutes lettres (ex. "mardi 1 septembre 2026"), sans heure.

*Rosée Matinale suit une disposition différente et propre — voir Partie 12.*

### 10.2 Sur les cartes d'aperçu (hubs et accueil)

**Ordre exact, sans exception :**
1. Ligne 1 : badge de catégorie seul (nom complet — "Que Dit la Bible", "La Vie Supérieure" — jamais de sigle QB/VS, même couleur de badge qu'auparavant).
2. Ligne 2 : date (format complet en toutes lettres, uniforme partout) + nombre de vues, sur la même ligne, espacement net entre les deux (pas collé), vues en violet.
3. Titre.
4. Extrait du chapeau, tronqué proprement, terminé par un lien inline **généré automatiquement par le code** (jamais saisi par Serge) : "…" puis "Lire la suite" (Que Dit la Bible) ou "Découvrir" (La Vie Supérieure) en couleur, dans le prolongement direct du même paragraphe — jamais de retour à la ligne forcé, jamais un bouton séparé.

**Tailles de police** : badge à 18px (taille du chapeau, nettement plus visible que la date/vues), titre/chapeau alignés sur la taille du corps de texte de l'article sur les hubs ; taille acceptée telle quelle sur l'accueil (légèrement plus petite, convention normale pour une carte compacte). Espacement d'environ 50% de plus entre badge et titre par rapport à l'origine.

**Image d'aperçu** : retirée entièrement des cartes (hubs et accueil) — aucun espace réservé, qu'une image existe ou non pour l'article. L'image reste utilisée uniquement dans la page article elle-même et dans l'image Open Graph.

**Nombre d'éléments par page** : maximum 4 publications par page, sur l'accueil ET les hubs, avec pagination (couleur de pagination vive, pas sombre).

### 10.3 Section "Autres articles similaires" (fin d'article)

- Titre neutre, sans nom de catégorie entre guillemets ("Autres articles similaires").
- Chaque carte : titre + chapeau (même pour une Rosée Matinale liée manuellement — repli automatique sur le début du corps ou le verset si pas de chapeau propre à ce type).
- Liens limités précisément au titre et au chapeau de chaque carte — jamais toute la zone/carte enveloppée dans un lien unique (métadonnées et espaces vides exclus du lien).
- Taille de police du titre/chapeau alignée sur le corps de texte de l'article en cours de lecture.

---

## PARTIE 11 — Pages légales, conformité et emails système

### 11.1 Mentions légales
Hébergeur (Vercel Inc., 440 N Barranca Ave #4133, Covina, CA 91723) complété. SIRET et Directeur de la publication conservés (obligatoires légalement). Ligne "à compléter" (adresse postale/email directeur) retirée à la demande de Serge.

### 11.2 Politique de confidentialité
Outils tiers précisés : MailerLite (newsletter), Resend (emails transactionnels), Stripe (paiement), Vercel (hébergement).

### 11.3 Bandeau de consentement cookies (RGPD)
Bannière Accepter/Refuser avant tout dépôt de cookie Google Analytics — construite, pas seulement mentionnée en texte.

### 11.4 Case de consentement newsletter — décision explicite
**La case RGPD ("J'accepte de recevoir la newsletter") n'est jamais cochée par défaut** — une case pré-cochée n'est pas un consentement valide au regard du RGPD (acte positif requis). Décision confirmée par Serge après discussion du risque légal.

### 11.5 Modalités de retour/remboursement
Texte reflétant le minimum légal français (14 jours de rétractation, remboursement sous 14 jours, frais de retour à la charge du client sauf produit défectueux) — à remplacer par des conditions plus généreuses une fois le circuit logistique réel (livres + Printful) en place.

### 11.6 Gabarit email — réinitialisation de mot de passe
HTML aux couleurs du site (en-tête dégradé violet/bleu, bouton pilule, logo blanc dédié centré dans l'en-tête), CSS inline (contrainte des clients email), texte en français. Bug de fond corrigé : le jeton de récupération est échangé contre une session une seule fois côté serveur (route `/auth/confirm`) avant l'affichage du formulaire — l'ancienne approche (session établie directement depuis l'URL) provoquait des erreurs "lien expiré" systématiques.

### 11.7 Restriction de copie/impression des articles
Clic droit désactivé et sélection de texte empêchée sur le corps des articles ; message de substitution à l'impression. **Dissuasion contre la copie occasionnelle, pas une protection technique absolue.**

---

## PARTIE 12 — Rosée Matinale — spécificités

### 12.1 URLs par jour
Passage de `/rosee-matinale?date=...` à `/rosee-matinale/JJ-JJ-JJ` (une vraie page par jour) — nécessaire pour que l'image Open Graph reflète le bon jour (limitation technique : `opengraph-image.tsx` ne reçoit jamais les paramètres de requête). `/rosee-matinale` (sans rien après) reste l'URL fixe du jour courant. Anciens liens `?date=` redirigés automatiquement.

### 12.2 Nouveau champ "Titre"
Champ facultatif, admin. Affichage sur la page publique, ordre exact : **titre (38px desktop / 26px mobile, le plus grand) → espacement net → badge "Rosée Matinale" (24px, police Manrope comme la date — jamais Fraunces/serif, pour ne pas se confondre visuellement avec le titre) → date (13px)**, le tout centré. Utilisé aussi dans la liste de sélection "articles similaires" et dans le message de partage (remplace l'ancien titre reconstruit depuis la date).

### 12.3 Archive
Section renommée "Rosées matinales précédentes". Pagination : 3 par page (pas 20).

### 12.4 Éditeur unique
Un seul éditeur, identique pour "Créer" et "Modifier" (le composant réutilisé est celui historiquement utilisé pour "Modifier" — centré, avec le bouton "Citation mise en exergue").

---

## PARTIE 13 — Éditeur de texte riche (admin) — fonctionnalités

### 13.1 Bouton "Citation" (blockquote)
Transforme le texte sélectionné sur place (liseré vertical coloré — violet sur Que Dit la Bible, bleu sur La Vie Supérieure). Utilisé volontairement par Serge pour styliser ses titres de section. Doit être supprimable proprement (toggle) sans affecter le texte d'origine.

### 13.2 Bouton "Citation mise en exergue" (pull quote) — distinct du précédent
Duplique le texte sélectionné dans un nouveau bloc séparé (agrandi, centré, couleur selon catégorie), sans toucher au texte d'origine qui reste inchangé dans le paragraphe. Déplaçable librement par Serge dans l'article (glisser-déposer), en conservant sa mise en forme. Supprimable proprement sans casser la mise en page.

### 13.3 Listes à puces/numérotées
Comportement fiable requis (l'implémentation `document.execCommand` étant connue pour son inconsistance, remplacée par une manipulation directe du HTML).

### 13.4 Barre d'outils
Taille standard et confortable (comparable à Google Docs/Notion), icônes SVG, surlignage de l'état actif pendant la frappe, **reste fixe à l'écran** pendant le défilement du texte.

### 13.5 Champ "Prière & Déclaration"
Texte unique (pas une liste répétable), section conditionnelle (visible uniquement si rempli), style italique + fond gris clair (`#F2F2F4`) sur la page publique. Disponible sur La Vie Supérieure et Que Dit la Bible (pas Rosée Matinale).

### 13.6 Thèmes/tags (catégorie libre, distincte de QB/VS/RM)
Système de liste librement gérée par Serge (voir §6.3), affichée publiquement sur chaque page d'article, juste avant les icônes de partage — positionnée en fin d'article, pas en tête de page. Interface compacte (étiquettes/chips avec retour à la ligne automatique, pas un empilement vertical d'un champ par thème), avec suppression fonctionnelle (croix) et recherche/autocomplétion plutôt qu'une liste à plat qui grossit indéfiniment.

### 13.7 Comportement de sauvegarde
Après "Enregistrer"/"Publier", rester sur l'écran d'édition avec un message de confirmation temporaire — jamais de redirection automatique vers la liste. Le retour à la liste reste un choix manuel de Serge. S'applique à tous les types de contenu de l'admin.

### 13.8 Catégorie modifiable après création
Le sélecteur QB/VS (dans la barre du haut de l'éditeur d'article) reste modifiable à tout moment, pas seulement à la création.
